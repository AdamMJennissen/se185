...
-----------------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h> //added standard library in order to use rand functions
#include <time.h>

/*----------------------------------------------------------------------------- // added * to start the comment
-                                Prototypes                                   -
------------------------------------------------------------------------------*/
...
void run_game(int computer_number);//added prototype for function
...
int main(int argc, char *argv[])
{
    ...
    while (prompt == 'y')     /* This line does not contain an error */
    {
        ...
        prompt = ask_to_play(played);//added e so variable was spelled correctly
...
int select_random_number()
{
    srand(time(NULL));
    // return rand() % 100;
    return ((rand() % 100)+1);//changed so the rand is within 1-100
...
void run_game(int computer_number)
{
    ...
    while (!correct)    /* This line does not contain an error */
    {
        ...
        else if (number == computer_number)//added = sign
        {
            printf("\nThe number was %d!\n", computer_number);
        ...
        // else if (number < computer_number);
        else if (number < computer_number)//removed ;

        {
            printf("\nYou guessed too low. Enter another guess.\n  -> ");
        } 
        // else
        else if (number > computer_number)//changed to else if statement
        {
