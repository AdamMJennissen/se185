...
//compile with ggc lab8_2.c -o lab8 -lncurses
//run with ./ds4rd.exe -d 054c:09cc -D DS4_BT -t -g -b | ./lab8 /*difficulty*/

void main(int argc, char *argv[])
{
    int t, temp_time, temp_time2;
    int T, C, X, S;
    ...
    else
    {
        ...
        srand(time(NULL));
        //converts command line argument to an int and generates the maze
        generate_maze(atoi(a));
        draw_maze();
        //spawns the avatar
        ...
        do
        {
            ...
            if (temp_time2 + 70 <= t)
            {
                ...
                if ((avg_x > .5) && (point_x > 0) && (MAZE[point_x - 1][point_y] != WALL))
                    point_x -= 1;
                else if ((avg_x < -.5) && (point_x < (COLUMNS - 1)) && (MAZE[point_x + 1][point_y] != WALL))
                    point_x += 1;
            }
            
            //move down
            if (temp_time + 250 <= t && (MAZE[point_x][point_y + 1] != WALL) &&  (MAZE[temp_x][point_y + 1] != WALL))
            {
                temp_time = t;
                point_y += 1;
            }
            ...
            if  ((MAZE[point_x + 1][point_y] == WALL) && (MAZE[point_x - 1][point_x] == WALL) && (MAZE[point_x][point_y + 1] == WALL))
            {
                endwin();
                printf("YOU LOSE. :(\n");
                return;
        ...
        } while (point_y != (ROWS - 1)); // Change this to end game at right time
